...
def run_ping1D_service(address: str="192.168.2.2", port: int=42070, buffer_size: int=2048, **config):
    """
    Opens a UDP connection to the AUV Ping1D service. This connection starts the Ping1D service route for collecting and reporting distance data. This script will send configuration data to the service and return the reported distance and confidence rating. Note: if n_samples > 1, then this value will be the average value recorded by the sensor
    ...
    Arguments:
        ...
        config: various configuration parameters for the Ping360 service and client script
            e.g. speed of sound, target angle/sweeps, parse data, log data, etc.

    Returns:
        The distance and confidence rating reported by the Ping1D sensor
    """
    ...
    _from_server = pk.loads(_udp_client_socket.recv(buffer_size))

    # Reading the dictionary sent by the server
    return (_from_server["distance"], _from_server["confidence"])
